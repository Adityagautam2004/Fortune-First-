  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-brand-border bg-card px-6 py-10 text-center text-muted-foreground">No payouts processed yet.</div>
      ) : (
        <>
          {/* Card list — mobile only, so every field is visible without side-scrolling. */}
          <div className="space-y-3 md:hidden">
            {payouts.map((p) => (
              <div key={p.id} className="rounded-2xl border border-brand-border bg-card p-4">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate font-medium text-foreground">{p.customer_name}</p>
                    <p className="truncate text-xs text-muted-foreground">{p.customer_email}</p>
                  </div>
                  <span className={`inline-flex shrink-0 items-center rounded-full px-3 py-1 text-xs font-semibold capitalize ${STATUS_STYLES[p.payout_status] || 'bg-muted text-muted-foreground'}`}>
                    {p.payout_status}
                  </span>
                </div>
                <div className="space-y-1.5 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-foreground">{MONTH_LABELS[p.month - 1]} {p.year}</span>
                  </div>
                  <div className="flex items-center justify-between gap-3">
