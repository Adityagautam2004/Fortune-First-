...
interface OperatingCapitalChartProps {
  reports: MonthlyReport[];
}

// Sourced from the monthly reports admin/board enter by hand — replaces the
// old "AUM Over Time" placeholder now that this data actually exists. The
// Y-axis pane is frozen (its own non-scrolling chart instance) while the
// plot area scrolls horizontally, showing the latest 12 months by default.
export function OperatingCapitalChart({ reports }: OperatingCapitalChartProps) {
  const chartData = useMemo(
    () =>
      [...reports]
        .sort((a, b) => a.year - b.year || a.month - b.month)
        .map((r) => ({ label: `${MONTH_LABELS[r.month]} ${r.year}`, value: Number(r.operating_capital_total) || 0 })),
    [reports]
  );
  ...
  // the badge shows the latest month's value, matching the reports list page.
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-brand-border bg-card p-4">
      <div className="mb-0.5 flex items-center justify-between">
        <h3 className="text-base font-bold text-foreground">Operating Capital Over Time</h3>
        <span className="rounded-lg border border-brand-border px-2.5 py-1 text-xs font-semibold text-foreground">
          Latest: {formatRupees(latest)}
        </span>
      </div>
      <p className="mb-3 text-xs text-muted-foreground">Operating Capital (₹) — scroll to see past months</p>

      {chartData.length === 0 ? (
        <div className="flex h-[260px] flex-col items-center justify-center gap-1 rounded-xl bg-brand-surface px-2 text-center">
          <LineChartIcon size={20} className="shrink-0 text-muted-foreground" />
        ...
        <div className="flex h-[260px] w-full">
          <div style={{ width: Y_AXIS_WIDTH, height: '100%', flexShrink: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 12, right: 0, left: 4, bottom: 8 }}>
                <YAxis
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 12, right: 16, left: 0, bottom: 8 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [formatRupees(Number(value)), 'Operating Capital']}
                  />
                  <Line
                    type="monotone"
                    dataKey="value"
