...
// everyone — scoping happens server-side in GET /board/investments. This is
// view-only: only super_admin can approve/reject (PATCH /admin/investments),
// so there's no decision UI here, only status.
export function BoardInvestmentsPage() {
  const [investments, setInvestments] = useState<BoardInvestmentRow[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    api
      .get('/board/investments', { params: { page, limit: 15, status: statusFilter === 'all' ? undefined : statusFilter } })
      .then((res) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-foreground">Investment</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Every investment submitted for your clients. Pending ones await admin approval.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-brand-border bg-card px-6 py-10 text-center text-muted-foreground">No investments found.</div>
      ) : (
        <>
          {/* Card list — mobile only, so every field is visible without side-scrolling. */}
          <div className="space-y-3 md:hidden">
            {investments.map((inv) => (
              <div key={inv.id} className="rounded-2xl border border-brand-border bg-card p-4">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <p className="font-medium text-foreground">{inv.customer_name}</p>
                  <span className={`inline-flex shrink-0 items-center rounded-full px-3 py-1 text-xs font-semibold capitalize ${STATUS_STYLES[inv.status] || 'bg-muted text-muted-foreground'}`}>
                    {inv.status}
                  </span>
                </div>
                <div className="space-y-1.5 text-sm">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-foreground">{formatDate(inv.investment_date)}</span>
                  </div>
                  <div className="flex items-center justify-between gap-3">
